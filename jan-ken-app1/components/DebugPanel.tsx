...
export function DebugPanel() {
  ...
  useEffect(() => {
    ...
    console.log = (...args: any[]) => {
      ...
      // Track matchmaking logs - ENHANCED: Track all matchmaking-related logs
      if (logStr.includes('[tryMatch]') || logStr.includes('[checkForMatch]') || logStr.includes('[joinQueue]') || logStr.includes('[leaveQueue]') || logStr.includes('[Matchmaking]') || logStr.includes('[getQueueCount]')) {
        let logType: MatchmakingLog['type'] = 'error';
        let step: string | undefined;
        
        if (logStr.includes('[tryMatch]')) {
          logType = 'tryMatch';
          if (logStr.includes('Step 1') || logStr.includes('Finding waiting players')) step = 'Step 1: Finding Players';
          else if (logStr.includes('Step 2') || logStr.includes('Verifying players')) step = 'Step 2: Verifying Players';
          else if (logStr.includes('Step 3') || logStr.includes('Updating status')) step = 'Step 3: Updating Status';
          else if (logStr.includes('Lock acquired')) step = 'Lock Acquired';
          else if (logStr.includes('Lock already held')) step = 'Lock Held (Skipped)';
          else if (logStr.includes('Match created successfully') || logStr.includes('✅✅✅')) {
            logType = 'matchFound';
            step = 'Match Created';
          } else if (logStr.includes('Not enough players')) step = 'Insufficient Players';
          else if (logStr.includes('last_seen')) step = 'Checking Player Activity';
          else if (logStr.includes('CRITICAL') || logStr.includes('❌')) step = 'Error Detected';
        } else if (logStr.includes('[checkForMatch]')) {
          logType = 'checkForMatch';
          if (logStr.includes('Fetching queue status')) step = 'Fetching Queue Status';
          else if (logStr.includes('Player still waiting')) step = 'Player Waiting - Attempting Match';
          else if (logStr.includes('Match created')) step = 'Match Found';
          else if (logStr.includes('Player not in queue')) step = 'Player Not In Queue';
          else if (logStr.includes('Game found')) step = 'Game Found';
          else if (logStr.includes('REJECTING GAME')) step = 'Game Rejected';
          else if (logStr.includes('❌') || logStr.includes('⚠️')) step = 'Error/Warning';
        } else if (logStr.includes('[joinQueue]')) {
          logType = 'joinQueue';
          if (logStr.includes('Checking existing queue')) step = 'Checking Existing Queue';
          else if (logStr.includes('Cleaning up old entries')) step = 'Cleaning Up Old Entries';
          else if (logStr.includes('Creating queue entry')) step = 'Creating Queue Entry';
          else if (logStr.includes('Queue entry created')) step = 'Queue Entry Created';
          else if (logStr.includes('Immediate match')) step = 'Attempting Immediate Match';
          else if (logStr.includes('❌') || logStr.includes('Error')) step = 'Error';
        } else if (logStr.includes('[leaveQueue]')) {
          logType = 'leaveQueue';
          if (logStr.includes('Leaving queue')) step = 'Leaving Queue';
          else if (logStr.includes('Successfully left')) step = 'Left Successfully';
          else if (logStr.includes('❌') || logStr.includes('Error')) step = 'Error';
        } else if (logStr.includes('[getQueueCount]')) {
          logType = 'checkForMatch'; // Use checkForMatch type for queue count
          if (logStr.includes('Queue count for betLevel')) step = 'Queue Count Calculated';
          else if (logStr.includes('❌') || logStr.includes('Error')) step = 'Queue Count Error';
        } else if (logStr.includes('[Matchmaking]')) {
          logType = 'checkForMatch';
          if (logStr.includes('Queue count updated')) step = 'Queue Count Updated';
          else if (logStr.includes('MATCH FOUND')) step = 'Match Found';
          else if (logStr.includes('Stopping all polling')) step = 'Stopping Polling';
          else if (logStr.includes('onMatchFound')) step = 'Calling onMatchFound';
        }
        ...
        // CRITICAL: Detect getQueueCount issues and incorrect counts
        if (logStr.includes('[getQueueCount]')) {
          // Detect errors
          if (logStr.includes('❌') || logStr.includes('Error')) {
            addIssue({
              id: `queue-count-error-${Date.now()}`,
              title: '⚠️ Queue Count Error',
              status: 'error',
              message: logStr,
              details: { 
                step, 
                data: args.length > 1 ? args.slice(1) : undefined,
                issue: 'queue_count_error',
                severity: 'critical',
              },
            });
          }
          
          // Detect queue count calculation details
          if (logStr.includes('Queue count for betLevel') && args.length > 1) {
            try {
              const logData = args[1];
              if (logData && typeof logData === 'object') {
                const countData = logData as any;
                // Check if count is incorrect (totalFound > uniqueCount significantly)
                if (countData.totalFound && countData.uniqueCount && countData.totalFound > countData.uniqueCount + 1) {
                  addIssue({
                    id: `queue-count-discrepancy-${Date.now()}`,
                    title: '⚠️ Queue Count Discrepancy Detected',
                    status: 'warning',
                    message: `Found ${countData.totalFound} entries but only ${countData.uniqueCount} unique players - duplicates detected`,
                    details: {
                      step,
                      totalFound: countData.totalFound,
                      validCount: countData.validCount,
                      uniqueCount: countData.uniqueCount,
                      betLevel: countData.betLevel,
                      players: countData.players,
                      issue: 'queue_count_discrepancy',
                      severity: 'warning',
                    },
                  });
                }
                
                // Check if any players have inactive last_seen but are still counted
                if (countData.players && Array.isArray(countData.players)) {
                  const inactivePlayers = countData.players.filter((p: any) => p.seconds_ago && p.seconds_ago > 15);
                  if (inactivePlayers.length > 0) {
                    addIssue({
                      id: `queue-count-inactive-${Date.now()}`,
                      title: '⚠️ Inactive Players in Queue Count',
                      status: 'error',
                      message: `${inactivePlayers.length} inactive player(s) found in queue count (last_seen > 15s)`,
                      details: {
                        step,
                        inactiveCount: inactivePlayers.length,
                        inactivePlayers: inactivePlayers,
                        betLevel: countData.betLevel,
                        issue: 'inactive_players_in_count',
                        severity: 'critical',
                      },
                    });
                  }
